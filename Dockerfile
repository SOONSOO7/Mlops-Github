...
# Alpine Linux는 경량화된 리눅스 배포판으로, 컨테이너 환경에 적합 => 가볍다 => ???
# => 빌드가 계속 반복되는데, 이미지 자체가 무거우면 빌드 속도가 느려집니다.
# ex) miniconda
...
# 이는 Python이 표준 입출력 버퍼링을 비활성화하게 하여, 로그가 즉시(실시간) 콘솔에 출력되게 합니다. 
...
# tmp에 넣는 이유? 컨테이너를 최대한 가볍게 유지하기 위해: 경량화
